...
public class DocAGnew {
	...
	public DocAGnew(attributenetwork an){
		this.an = an;
...
//		System.out.print(kCluster.size());
	...
	public Pair CalculateSimilarity(Cluster a, Cluster b){
...
//    				Pair simPair = CalculateSim(newClusters.get(i), StructureClusList.get(j));
    				Pair simPair = CalculateSimilarity(newClusters.get(i), StructureClusList.get(j));
...
//		- codingScheme.CCCluster(mergeCandidate.member[0]) - codingScheme.CCCluster(mergeCandidate.member[1]);
		double ccinitial = codingScheme.ccClusterStructure 	
		- codingScheme.CCCluster(mergeCandidate.member[0]) - codingScheme.CCCluster(mergeCandidate.member[1]);
...
//		Output(RemainC);
	...
	/*public ArrayList<Cluster> FindBigBlock(Cluster mergeC, ArrayList<Cluster> ClusList){
		int[][] adjFMergeC = new int[mergeC.Nodes.size()][FullFeature.size()];
		for(int i = 0; i < mergeC.Nodes.size(); i ++){
			for(int j = 0 ; j < FullFeature.size(); j ++){
				adjFMergeC[i][j] = AdjFeature[mergeC.Nodes.get(i).id][FullFeature.get(j)];
			}
		}
...
//		}
    	...
    	for(int i = 0; i < ClusList.size() - 1; i ++){
    		for(int j = i + 1; j < ClusList.size(); j ++){  			
    			Pair simPair = CalculateSimilarity(ClusList.get(i), ClusList.get(j));
    			simQueue.add(simPair);
    		}
    	} 
...
//		an = ea.readNetwork("data/syn/new2clusterGraph");
...
//		sc.next();
		...
		DocAGnew docag = new DocAGnew(an);
