...
public class IROCmain {
	...
	public IROCmain(attributenetwork an){
		this.an = an;
...
//		ClusList = InitialClusList;
		...
		for(int i = 0; i < ClusList.size(); i++){
			Output(ClusList.get(i));
		}
		
		
		/** For each pair of clusters, calculate their similarities and rank */
		double[][] simMatrix = new double[ClusList.size()][ClusList.size()];
...
//    			Pair simPair = CalculateSim(ClusList.get(i), ClusList.get(j));
    			...
    			simMatrix[i][j] = simPair.similarity;
    			simMatrix[j][i] = simPair.similarity;
    			simQueue.add(simPair);
    		}
    	} 
    	...
    	for(int K = 2; K < 3; K ++){
    		...
    		KMeans km = new KMeans(K, simMatrix);
    		int[] label = km.getLabel();
    		for(int i = 0; i < label.length; i ++){
    			System.out.println(label[i]);
    		}
    		ArrayList<Cluster> newClusList = new ArrayList<Cluster>();
    		for(int i = 0; i < K; i ++){
    			ArrayList<Cluster> clusK = new ArrayList<Cluster>();
    			for(int j = 0; j < label.length; j ++){
    				if(label[j] == i){
    					clusK.add(ClusList.get(j));
    				}
    			}
    			Cluster newCluster = new Cluster(an);
    			ArrayList<vertex> groupNodes = new ArrayList<vertex>();
    			for(int k = 0; k < clusK.size(); k ++){
    				for(int l = 0; l < clusK.get(k).Nodes.size(); l ++){
    					if(! groupNodes.contains(clusK.get(k).Nodes.get(l))){
    						groupNodes.add(clusK.get(k).Nodes.get(l));
    					}
    				}
    			}
    			newCluster.Nodes.addAll(groupNodes);
    			newClusList.add(newCluster);
    		}    		
...
//		System.out.println(codingScheme.CalculateCC(ClusList));
		...
		int K = 2;
		while(true){
			ArrayList<Cluster> newClusList = Kmeans(ClusList,K);
			newClusList = FindSubspace(newClusList);
			double oldCost = codingScheme.CalculateCC(newClusList);
			double newCost  = 0.0;
			for(Cluster clus : newClusList){
				for(vertex v : clus.Nodes){
					clus.Nodes.remove(v);
					FindClusterSubspace(clus, newClusList);
					newCost = codingScheme.CalculateCC(newClusList);
					if(newCost > oldCost){
						clus.Nodes.add(v);
						continue;
					}
						...
						oldCost = newCost;						
						for(Cluster clusm : newClusList){
							if(! clusm.equals(clus)){
								clusm.Nodes.add(v);
								FindClusterSubspace(clusm, newClusList);
								newCost = codingScheme.CalculateCC(newClusList);
								if(newCost > oldCost)
									clus.Nodes.remove(v);															
							}
	...
	public ArrayList<Cluster> Kmeans(ArrayList<Cluster> ClusList, int K){
		ArrayList<Cluster> newClusList = new ArrayList<Cluster>();   // The final rough clusters
		IROCmain ir = new IROCmain(an);
		
		/*Random choose initial center */
		Random r = new Random();
		ArrayList<Cluster> centerClus = new ArrayList<Cluster>();
		for(int i = 0; i < K; i ++){
			int selectID = r.nextInt(ClusList.size());
			if(! centerClus.contains(ClusList.get(selectID)))
				centerClus.add(ClusList.get(selectID));			
		}		
		ArrayList<Cluster> selectCenter = new ArrayList<Cluster>();  // The changing centers	
		ArrayList<ArrayList<Cluster>> group = new ArrayList<ArrayList<Cluster>>();
		while(true){			
			/*Assignment*/
			int[] clusMark = new int[ClusList.size()];
			for(int i = 0; i < clusMark.length; i ++){
				clusMark[i] = -1;
			}
			for(int i = 0; i < ClusList.size(); i ++){
				double sim = Double.MAX_VALUE;
				for(int j = 0; j < centerClus.size(); j ++){
					Pair simPair = CalculateSimEntropy(ClusList.get(i), centerClus.get(j));
					if(simPair.similarity < sim){
						sim = simPair.similarity;
						clusMark[i] = j;
					}						
				}
			}
			group.clear();
			for(int i = 0; i < K ; i ++){
				ArrayList<Cluster> groupClus = new ArrayList<Cluster>();
				for(int j = 0; j < clusMark.length; j ++){
					if(clusMark[j] == i){
						groupClus.add(ClusList.get(j));
					}
				}
				group.add(groupClus);
			}
			...
			for(int i = 0; i < group.size(); i++){
				...
				Cluster groupCenter = new Cluster(an);
				for(Cluster clus1 : group.get(i)){
					double sum = 0.0;					
					for(Cluster clus2 : group.get(i)){
						Pair simPair = CalculateSimEntropy(clus1, clus2);
						sum += simPair.similarity;
					}
					if(sum < simmin){
						simmin = sum;
						groupCenter.clear();
						groupCenter = clus1.copy();
					}
				}
				selectCenter.add(groupCenter);
			}
			boolean flag = false;
			for(Cluster c1 : selectCenter){	
				flag = false;
				if(centerClus.contains(c1))
					flag = true;								
			}
			if(flag == true)
				break;
			for(int i = 0; i < selectCenter.size(); i ++){
				ir.Output(selectCenter.get(i));
			}
			centerClus.clear();
			centerClus.addAll(selectCenter);
		}
		
		for(int i = 0; i < group.size(); i ++){
			int[] tempLabel = new int[ClusList.size()];
			for(Cluster clus : group.get(i)){
				for(vertex v : clus.Nodes){
					tempLabel[v.id] = 1;
				}
			}
			Cluster newClus = new Cluster(an);
			for(int j = 0; j < tempLabel.length; j ++){
				if(tempLabel[j] == 1){
					vertex newV = new vertex(j);
					newClus.Nodes.add(newV);
				}
			}
			newClusList.add(newClus);
		}		
		return newClusList;
	}
...
//		sc.next();
		...
		IROCmain docag = new IROCmain(an);
