...
public class Clustering {
	...
	public Clustering(attributenetwork an) {
		this.an = an;
...
//					Output(selectCluster);
			...
			kCluster.add(selectCluster);
			ClusList.remove(selectCluster);
			for(vertex v : selectCluster.Nodes){
				Label[v.id] = 1;
			}
			int sum = 0;
...
//		System.out.print(kCluster.size());
	...
	public Pair CalculateSimMutualInformation(Cluster a, Cluster b){
		Pair simPair = new Pair();
		simPair.member[0] = a;
		simPair.member[1] = b;
		simPair.structureCN = NodeCommonNeighbor(a.Nodes, b.Nodes);
		...
		for(int i = 0; i < simPair.structureCN.size(); i ++){
			for(int j = 0; j < simPair.structureCN.size(); i ++){
		...
		double structuresimilarity = AdjEntropy(a.adjStructure) + AdjEntropy(b.adjStructure) - AdjEntropy(adjCN);

		double featuresimilarity = 0.0;
		int[][] featurematrixCN = new int[simPair.structureCN.size()][simPair.featureCN.size()];
		for (int i =0 ; i < simPair.structureCN.size(); i ++ ) {
			for(int j = 0; j < simPair.featureCN.size(); j ++){
				featurematrixCN[i][j] = simPair.structureCN.get(i).feature.get(j);
			}
		}		
		featuresimilarity = FeatureEntropy(a.adjFeature,a.Features) + FeatureEntropy(b.adjFeature,b.Features) - FeatureEntropy(featurematrixCN,simPair.featureCN);		
		simPair.similarity = structuresimilarity + featuresimilarity; 
		return simPair;
	}
	...
	public double FeatureEntropy(int[][] featurematrix, ArrayList<Integer> feature){
		double ent = 0.0;
		for(Integer f : feature){
			int[] cate = new int[an.numCategory[f]];
			for(int i = 0; i < featurematrix.length; i ++){
				cate[featurematrix[i][f]] ++;
			}
...
//			}
	...
	public ArrayList<Cluster> ModifyClusterAll(ArrayList<Cluster> ClusList, Pair mergeCandidate){
		CodingCost cc = new CodingCost(an);
		ArrayList<Cluster> newCluster = new ArrayList<Cluster>();	
		double cc_compare = cc.CalculateCC(ClusList);    // Compared coding cost
		HashMap<ArrayList<Cluster>, Double> Cases = new HashMap<ArrayList<Cluster>, Double>();
		...
		newClusters1.add(C1);
		if(C1.Nodes.size()>0){
			case1ClusList.addAll(0,newClusters1);
		...
		C21.Nodes.addAll(0,NodesC21);
		C22.Nodes.addAll(0,NodesC22);	
		newClusters2.add(C21);
		newClusters2.add(C22);	
		if(C21.Nodes.size() > 0 && C22.Nodes.size() > 0){
			case2ClusList.addAll(newClusters2);
		...
		C31.Nodes.addAll(0,NodesC31);
		C32.Nodes.addAll(0,NodesC32);	
		newClusters3.add(C31);
		newClusters3.add(C32);
		if(C31.Nodes.size() > 0 && C32.Nodes.size() > 0){
			case3ClusList.addAll(newClusters3);
		...
		C41.Nodes.addAll(0,NodesC41);		
		C42.Nodes.addAll(0,NodesC42);
		C43.Nodes.addAll(0,CNNodes);
		newClusters4.add(C41);
		newClusters4.add(C42);
		newClusters4.add(C43);
		if(C41.Nodes.size() > 0 && C42.Nodes.size() > 0 && C43.Nodes.size() > 0){
...
//			}
	...
	public ArrayList<Cluster> ModifyClusterwithoutorder(ArrayList<Cluster> ClusList, Pair mergeCandidate){
		...
		if(C1.Nodes.size()>0){
			case1ClusList.addAll(newClusters1);
			case1ClusList = FindSubspacewithoutOrder(newClusters1, case1ClusList);		
			double cccase1 = cc.CalculateCC(case1ClusList);
			Cases.put(newClusters1, cccase1);
		}
		...
		if(C21.Nodes.size() > 0 && C22.Nodes.size() > 0){
			case2ClusList.addAll(newClusters2);
			case2ClusList = FindSubspacewithoutOrder(newClusters2, case2ClusList);		
			double cccase2 = cc.CalculateCC(case2ClusList);
			Cases.put(newClusters2, cccase2);
		}
		...
		if(C31.Nodes.size() > 0 && C32.Nodes.size() > 0){
			case3ClusList.addAll(newClusters3);
			case3ClusList = FindSubspacewithoutOrder(newClusters3, case3ClusList);		
			double cccase3 = cc.CalculateCC(case3ClusList);
			Cases.put(newClusters3, cccase3);	
		}
		...
		if(C41.Nodes.size() > 0 && C42.Nodes.size() > 0 && C43.Nodes.size() > 0){
			case4ClusList.addAll(newClusters4);
			case4ClusList = FindSubspacewithoutOrder(newClusters4, case4ClusList);		
			double cccase4 = cc.CalculateCC(case4ClusList);
			Cases.put(newClusters4, cccase4);	
		}	
...
//					Output(cl);
	...
	public ArrayList<Cluster> PostProcessing(ArrayList<Cluster> ClusList){		
		...
		double cc = cd.CalculateCC(ClusList);         // total coding cost		
...
//					Output(modiclus);
					modi.add(modiclus);	
					ClusList.remove(modiclus);	
					ClusList.add(modiclus);
					modi = NewFindSubspace(modi, ClusList);
//					modi = FindSubspacewithoutOrder(modi, ClusList);
//					Output(modiclus);					
					double ccnew = cd.CalculateCC(ClusList);				
					if(ccnew > cc){	
						ClusList.remove(modiclus);
...
//		Output(scluster);
	...
	public ArrayList<Cluster> ModifyNodes(ArrayList<Cluster> ClusList){
		CodingCost CC = new CodingCost(an);
		double cc = CC.CalculateCC(ClusList);
		for(int i = 0; i < ClusList.size(); i ++){
			for(int j =0; j < ClusList.size(); j ++){
				if(i != j){
					Cluster clu1 = ClusList.get(i);
					Cluster clu2 = ClusList.get(j);
					for(vertex v : clu1.Nodes){
						clu1.Nodes.remove(v);
						clu2.Nodes.add(v);
						double cctemp = CC.CalculateCC(ClusList);
						if(cctemp < cc){
							cc = cctemp;
						}
						else{
	...
	public ArrayList<Cluster> FinalSubspace(ArrayList<Cluster> ClusList){
		ArrayList<Cluster> finalClusList = new ArrayList<Cluster>();    // Final Clusters with subspaces
		CodingCost cc = new CodingCost(an);
		ArrayList<ArrayList<ArrayList<Integer>>> allOrderFeature = new ArrayList<ArrayList<ArrayList<Integer>>>(); // Feature order of all clusters
		for(Cluster c : ClusList){
			c.Features.clear(); // Clear subspace of all clusters
			ArrayList<ArrayList<Integer>> clusFeatureOrder = FindClusterFeatureOrder(c);
			allOrderFeature.add(clusFeatureOrder);
		}
		
		ArrayList<ArrayList<Integer>> candidateFeature = new ArrayList<ArrayList<Integer>>(); // Feature of each clusters going to be chosen 
		double ccfinal = cc.CalculateCC(ClusList);  // Final minimum coding cost	
		// Add feature to candidate
		for(int i = 0; i < ClusList.size(); i ++){			
			candidateFeature.add(i, allOrderFeature.get(i).get(0));
			allOrderFeature.get(i).remove(0);
		}
		
		    
		// All possible feature choose the one with minimum coding cost
		while(true){			   
			double[] cost = new double[ClusList.size()]; // cost of candidate		
			for(int i =0; i <candidateFeature.size(); i ++){
				if(candidateFeature.get(i) != null){					
					ClusList.get(i).Features.addAll(candidateFeature.get(i));
//					for(Cluster t : ClusList)
//						Output(t);
					double costtemp = cc.CalculateCC(ClusList);
					cost[i] = costtemp;
					ClusList.get(i).Features.removeAll(candidateFeature.get(i));
				}
				else
					cost[i] = Double.MAX_VALUE;
			}
			// Find feature with minimum coding cost
			...
			ClusList.get(minid).Features.addAll(candidateFeature.get(minid));
			if(!allOrderFeature.get(minid).isEmpty()){
				candidateFeature.set(minid, allOrderFeature.get(minid).get(0));
				allOrderFeature.get(minid).remove(0);
			}else
				candidateFeature.set(minid, null);
				
			
			double ccnew  = minvalue; // Coding cost of new added feature
//			for(Cluster t : ClusList)
//				Output(t);
			if(ccnew < ccfinal){
			   ccfinal = ccnew;
			   finalClusList.clear();
			   finalClusList.addAll(ClusList);
			}
			int label = 0;
			for(int i = 0; i < candidateFeature.size(); i ++){
				if(candidateFeature.get(i) == null)
			...
			if(label == candidateFeature.size())
				break;
		}
		return finalClusList;
	}
	
	public ArrayList<Cluster> PostProcessingnew(ArrayList<Cluster> ClusList){		
		NewCodingCost cd = new NewCodingCost(an);
		double cc = cd.CalculateCC(ClusList);         // total coding cost		
...
//					Output(modiclus);
					...
					ClusList.remove(modiclus);	
					ClusList.add(modiclus);
					ClusList = FinalSubspace(ClusList);					
//					Output(modiclus);					
					double ccnew = cd.CalculateCC(ClusList);				
					if(ccnew > cc){	
						ClusList.remove(modiclus);
...
//					Output(modiclus);
	...
	public static void main(String[] args){
		...
		IO ea = new IO();		
		an = ea.readNetwork("data/syn/3clusterGraph_0_40_30_40_60_40");
		System.out.println("Read Attributed Network");
		Clustering clustering = new Clustering(an);
		...
		for(Cluster clus : ClusList)
			clustering.Output(clus);
		ClusList = clustering.NewFindSubspace(ClusList, ClusList);
		for(Cluster clus : ClusList)
			clustering.Output(clus);
	}
}
